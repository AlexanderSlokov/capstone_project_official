...
from localgpt_llama2.config.configurations import CONTEXT_WINDOW_SIZE, MAX_NEW_TOKENS, MODELS_PATH, N_BATCH, N_GPU_LAYERS
...
def load_quantized_model_gguf_ggml(model_id, model_basename, device_type, logging):
    try:
        ...
        kwargs = {
            ...
            "n_batch": N_BATCH,  # Thiết lập dựa trên RAM của GPU & CPU
            "use_mmap": True,  # Sử dụng mmap để tải
        }

        if device_type.lower() == "cuda":
            kwargs["n_gpu_layers"] = N_GPU_LAYERS  # Thiết lập số lớp GPU để sử dụng

        # Khởi tạo mô hình LlamaCpp
...
def load_full_model(model_id, model_basename, device_type, logging):
    if device_type.lower() in ["mps", "cpu"]:
        ...
        model = AutoModelForCausalLM.from_pretrained(
            model_id,
            device_map={"": "cuda"},  # Thay đổi này đảm bảo mô hình được nạp vào GPU
